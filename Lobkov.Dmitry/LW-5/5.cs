...
namespace _5
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if(select == 1)
            {
                Sportsman jumper = new Jumper(firstname, lastname, middlename, record);
                jumper.ShowInfo();
                jumper.KindOfSports = "Jumper";
                jumper.ChangeParameters();
                jumper.PersonalRecord(jumper);
                Console.WriteLine($"{jumper.IsOutstanding}");
            }

            else if (select == 2)
            {
                Sportsman swimmer = new Swimmer(firstname, lastname, middlename, record);
                swimmer.KindOfSports = "Swimmer";
                Console.WriteLine($"{swimmer.ShowFullname}");
                swimmer.ShowSport();
                swimmer.CriminalRecord();
            }

            else if (select == 3)
            {
                Sportsman weghtlifter = new Weghtlifter(firstname, lastname, middlename, record);
                weghtlifter.KindOfSports = "Weghtlifter";
                Console.WriteLine($"{weghtlifter.IsDoping}");
                weghtlifter.PersonalRecord(weghtlifter);
                Console.Write("Lastname: ");
                weghtlifter["lastname"] = Console.ReadLine();
                Console.WriteLine($"Current age: {weghtlifter.Age} years\nEnter actual birth year: ");
                int.TryParse(Console.ReadLine(), out birthYear);
                weghtlifter.Age = birthYear;
            }
        }
    }
}