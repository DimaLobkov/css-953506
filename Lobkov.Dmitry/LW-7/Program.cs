...
namespace _7
{
    class Fraction : IComparable, IEquatable<Fraction>
    {
        ...
        public static void Initialization(string input, out Fraction number)
        {
            int numerator, denominator;
            string[] cell = input.Split(' ', '/');
            if (cell.Length == 1 && Int32.TryParse(cell[0], out numerator))
            {
                number = new Fraction(numerator, 1);
                return;
            }
            else if (cell.Length == 2 && Int32.TryParse(cell[0], out numerator) && Int32.TryParse(cell[1], out denominator) && denominator > 0)
            {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Fraction.Initialization(input, out firstNum);
            Console.Write("Second number: ");
            input = Console.ReadLine();
            Fraction.Initialization(input, out secondNum);
            do
            {
                int select;
